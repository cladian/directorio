...
/**
 * This is the model class for table "tiposcontacto".
 *
 * The followings are the available columns in table 'tiposcontacto':
 * @property integer $id
...
class Tiposcontacto extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('ismail, status', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>45),
			array('rdate, udate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, ismail, rdate, udate, status', 'safe', 'on'=>'search'),
			// Valores por defecto
	...
	public function relations()
	{
		...
		return array(
			'contactoses' => array(self::HAS_MANY, 'Contactos', 'tiposcontacto_id'),
		);
	}
