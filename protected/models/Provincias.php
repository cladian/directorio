...
class Provincias extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('code, status', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>45),
			array('rdate, udate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, code, rdate, udate, status', 'safe', 'on'=>'search'),
			// Valores por defecto
	...
	public function relations()
	{
		...
		return array(
			'coberturases' => array(self::HAS_MANY, 'Coberturas', 'provincias_id'),
			'consorcioses' => array(self::HAS_MANY, 'Consorcios', 'provincias_id'),
			'registroses' => array(self::HAS_MANY, 'Registros', 'provincias_id'),
		);
	}
