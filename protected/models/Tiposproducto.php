...
/**
 * This is the model class for table "tiposproducto".
 *
 * The followings are the available columns in table 'tiposproducto':
 * @property integer $id
...
class Tiposproducto extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, consorcios_id', 'required'),
			array('status, consorcios_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>45),
			array('description', 'length', 'max'=>100),
			array('rdate, udate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, rdate, udate, status, consorcios_id', 'safe', 'on'=>'search'),
			// Valores por defecto
	...
	public function relations()
	{
		...
		return array(
			'productoses' => array(self::HAS_MANY, 'Productos', 'tiposproducto_id'),
    ...
    public function searchByConsorcio($id)
    {
        // @todo Please modify the following code to remove attributes that should not be searched.
...
public function searchCons()
	{
		...
		$criteria->compare('udate',$this->udate,true);
		$criteria->compare('status',$this->status);
		$criteria->compare('consorcios_id',Yii::app()->user->getState('CONSORCIOID'));
